...
memberships_query = <<SPARQL
  ...
  {
    ?item p:P39 ?ps .
    ?ps ps:P39/wdt:P279* wd:Q17633943 ; pq:P2937 ?term .
...
resp = Scraped::Request.new(url: url).response

data = Results.new(response: resp).memberships
if ENV['MORPH_DEBUG']
  puts "Server = #{resp.headers['x-served-by']}: #{data.count} results" 
  puts data.map(&:compact).map(&:sort).map(&:to_h)
end
